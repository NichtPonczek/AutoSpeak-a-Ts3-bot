...
class name_day
{
	...
	static public function before_clients()
	{
		...
		$AutoSpeak::check_error($query->channelEdit(self::$cfg['channel_id'], array('channel_name' => str_replace('[NAME]', explode(',', $name_day[date("m-d")])[rand(0, count(explode(',', $name_day[date("m-d")]))-1)], self::$cfg['channel_name']))), self::$name, self::$cfg['channel_id'], true);
	}
}
?>