...
class generate_banner
{
	...
	static private function admins_online(&$img)
	{
		global $query, $clients;

		$cf = self::$cfg['admins_online'];
		...
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen($count)/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", $count);

	}

	static private function clients_online(&$img)
	{
		global $server_info;

		$cf = self::$cfg['clients_online'];
		
		$online = $server_info['virtualserver_clientsonline'] - $server_info['virtualserver_queryclientsonline'];
		
		if($cf['show_max_clients'])
			$online .= " / ".$server_info['virtualserver_maxclients'];
		
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen($online)/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", $online);
	}

	static private function record_online(&$img)
	{
		$cf = self::$cfg['record_online'];

		$record = @array_reverse(json_decode(file_get_contents("include/cache/record_online.txt"), true));
		$record_online = $record[0]['online'];

		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen($record_online)/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", $record_online);
	}

	static private function clock(&$img)
	{
		$cf = self::$cfg['clock'];
		
		$clock = date('H:i');
 
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen($clock)/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", $clock);
	}

	static private function channels_count(&$img)
	{
		global $server_info;

		$cf = self::$cfg['channels_count'];
		$count = $server_info['virtualserver_channelsonline'];
	...
	static private function name_day(&$img)
	{
		$cf = self::$cfg['name_day'];
 
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen(self::get_name())/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", self::get_name());
	}

	static private function fanpage_likes(&$img)
	{
		$cf = self::$cfg['fanpage_likes'];
 
		$likes = json_decode(file_get_contents('https://graph.facebook.com/'.$cf['page_id'].'?access_token='.$cf['access_token'].'&fields=name,fan_count'));
 
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen($likes->fan_count)/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", $likes->fan_count);
	}
	
	static private function uptime(&$img)
	{
		global $server_info;
		
		$cf = self::$cfg['uptime'];
	
		$uptime = round($server_info['virtualserver_uptime']/3600/24);
 
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen($uptime)/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", $uptime);
	}
	
	static private function generate_date(&$img)
	{
		$cf = self::$cfg['date'];
 
		ImageTTFText($img, $cf['co-ordinates'][0], $cf['co-ordinates'][1], $cf['co-ordinates'][2]-(imagefontwidth($cf['co-ordinates'][0])*strlen(date($cf['format']))/2), $cf['co-ordinates'][3], 
		ImageColorAllocate($img, $cf['color'][0], $cf['color'][1], $cf['color'][2]), "include/cache/fonts/".$cf['font'].".ttf", date($cf['format']));
	}
	...
	static public function before_clients()
	{
		...
		$image = ImageCreateFromPng(self::$cfg['image_file'][((self::$banner_number++)%count(self::$cfg['image_file']))]);
		imagealphablending($image, true);
		
		if(self::$cfg['admins_online']['enabled'])
			self::admins_online($image);

		if(self::$cfg['clients_online']['enabled'])
			self::clients_online($image);

		if(self::$cfg['record_online']['enabled'])
			self::record_online($image);

		if(self::$cfg['clock']['enabled'])
			self::clock($image);
	
		if(self::$cfg['channels_count']['enabled'])
			self::channels_count($image);

		if(self::$cfg['name_day']['enabled'])
			self::name_day($image);

		if(self::$cfg['fanpage_likes']['enabled'])
			self::fanpage_likes($image);
		
		if(self::$cfg['uptime']['enabled'])
			self::uptime($image);
		
		if(self::$cfg['date']['enabled'])
			self::generate_date($image);
		
   	 	imagepng($image, self::$cfg['target_image_file']); 
    	imagedestroy($image);
	}

	static private function get_name()
	{
		$name_day = array
		(
			//STYCZEN
