...
class top_idle_time
{
	...
	static public function before_clients()
	{
		...
		$result = $query_sql->query('SELECT `client_dbid`,`client_clid`,`client_uid`,`client_nick`,`server_groups`,`idle_time_record` FROM `clients` ORDER BY `idle_time_record` DESC LIMIT '.(5*self::$cfg['records']))->fetchAll(PDO::FETCH_ASSOC);
		...
		foreach($result as $user)
		{
			...
			if(!$AutoSpeak::has_group($user['server_groups'], self::$cfg['ignored_groups']))
				$desc .= "[list][*] [size=15] ".$count++.". [/size] [size=9][b]  Nick: [URL=client://".$user['client_clid']."/".$user['client_uid']."]".$user['client_nick']."[/url]".$AutoSpeak::show_link($user['client_dbid'])." → ".$AutoSpeak::convert_time($user['idle_time_record']/1000, self::$cfg['time_settings'])."[/size][/list]";
		}
